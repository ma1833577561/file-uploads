...
const _$=(select,ele)=>{
		if(!ele) return document.querySelector(select)
		return ele.querySelector(select)
	}

/* 基于FORM-DATA实现文件上传 */
(function () {
    let upload = _$('#upload1'),
        upload_inp =_$('.upload_inp',upload),
        upload_button_select =_$('.upload_button.select',upload),
        upload_button_upload =_$('.upload_button.upload',upload),
        upload_tip =_$('.upload_tip', upload),
        upload_list =_$('.upload_list',upload);
    let _file = null;
    ...
    upload_button_upload.addEventListener('click', function () {
        ...
        service.post('/upload_single', formData).then(data => {
            if (+data.code === 0) {
...
(function () {
    ...
    upload_inp.addEventListener('change', async function () {
        ...
        try {
            data = await service.post('/upload_single_base64', {
...
(function () {
    ...
    upload_button_upload.addEventListener('click', async function () {
        ...
        service.post('/upload_single_name', formData).then(data => {
            if (+data.code === 0) {
                alert(`文件已经上传成功~~,您可以基于 ${data.servicePath} 访问这个资源~~`);
                return;
            }
...
(function () {
    ...
    upload_inp.addEventListener('change', async function () {
        ...
        try {
            ...
            data = await service.post('/upload_single', formData, {
...
(function () {
    ...
    upload_button_upload.addEventListener('click', async function () {
        ...
        _files = _files.map(item => {
            ...
            return service.post('/upload_single', fm, {
                onUploadProgress(ev) {
                    // 检测每一个的上传进度
                    if (curSpan) {
...
(function () {
    ...
    const uploadFile = async file => {
        ...
        try {
            ...
            data = await service.post('/upload_single', fm);
            if (+data.code === 0) {
                alert(`恭喜您，文件上传成功，您可以基于 ${data.servicePath} 访问该文件~~`);
                return;
            }
...
(function () {
    ...
    upload_inp.addEventListener('change', async function () {
        ...
        // 获取已经上传的切片信息
        try {
            data = await service.get('/upload_already', {
                params: {
                    HASH
                }
            });
        ...
        const complate = async () => {
            ...
            upload_progress_value.style.width = `100%`;
            try {
                data = await service.post('/upload_merge', {
                    HASH,
                    count
                }, {
                    headers: {
        ...
        chunks.forEach(chunk => {
            ...
            service.post('/upload_chunk', fm).then(data => {
                if (+data.code === 0) {
                    complate();
                    return;
                }
